...
import { dashboardApi, itineraryApi } from '../services/api'
...
export default function DashboardPage() {
  ...
  const handleDeleteTrip = async (trip) => {
    ...
    try {
      await itineraryApi.delete(trip.id)
      await loadDashboard()
    } catch (err) {
      ...
      <main className="mx-auto max-w-[var(--spacing-container-max)] px-[var(--spacing-margin-mobile)] py-12 md:px-[var(--spacing-margin-desktop)]">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-[var(--spacing-gutter)] lg:grid-cols-12">
            <TripsSection trips={trips} pastTrips={pastTrips} onDeleteTrip={handleDeleteTrip} />
